...
async function udpSweepAll(subnet) {
    console.log(`Starting udp sweep on subnet: ${subnet}.0/24`);
...
async function findUdpPorts(host, commonPorts = [
    ...
    console.log(`Starting udp sweep on host: ${host}`);
    ...
    console.log("UDP sweep complete!", grouped);
    Object.entries(grouped).forEach(([host, ports]) => {
        console.log(`${host} → ${ports.join(', ')}`);
    });
}

async function findAllUdpPorts(host) {
...
module.exports = { udpSweep, findUdpPorts, udpSweepAll, findAllUdpPorts };
