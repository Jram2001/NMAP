...
async function tcpProbeDecode(sourceIP, targetIP) {
    ...
    //Temprory
    // promises.push(sendPacketAndListen(sourceIP, targetIP, 53, 1000, tcpPackets));
    // promises.push(sendPacketAndDecode(sourceIP, targetIP, 53, 1000, tcpPackets[1]));
    ...
    let ttl = extractOptions(results[0]?.decoded?.options);
    ...
    console.log(matchOSFingerprint(captured));
